...
class AdminService {
  ...
  async getAllClassSchedules(filters: {
    course_id?: number;
    semester?: number;
    year?: number;
    batch?: string;
  ...
  async createClassSchedule(schedule: ClassScheduleCreateRequest): Promise<ClassScheduleResponse> {
  ...
  async updateClassSchedule(id: number, schedule: ClassScheduleUpdateRequest): Promise<ClassScheduleResponse> {
  ...
  async updateCourse(id: number, course: CourseUpdateRequest): Promise<CourseResponse> {
    ...
    if (!response.ok) {
      const error = await response.json();
      throw new Error(error.detail || `Failed to update course: ${response.statusText}`);
    }
    return response.json();
  }
  ...
  async createProgram(program: ProgramCreateRequest): Promise<ProgramResponse> {
    ...
    if (!response.ok) {
      const error = await response.json();
      throw new Error(error.detail || `Failed to create program: ${response.statusText}`);
    }
    return response.json();
  }

  async updateProgram(id: number, program: ProgramUpdateRequest): Promise<ProgramResponse> {
    ...
    if (!response.ok) {
      const error = await response.json();
      throw new Error(error.detail || `Failed to update program: ${response.statusText}`);
    }
    return response.json();
  }
  ...
  async createExamSchedule(schedule: ExamScheduleCreateRequest): Promise<ExamScheduleResponse> {
    const response = await fetch(`${API_BASE_URL}/schedules`, {
      method: 'POST',
      headers: await this.getAuthHeaders(),
      body: JSON.stringify(schedule)
    ...
    if (!response.ok) {
      const error = await response.json();
      throw new Error(error.detail || `Failed to create exam schedule: ${response.statusText}`);
    }
    return response.json();
  }

  async updateExamSchedule(id: number, schedule: Partial<ExamScheduleCreateRequest>): Promise<ExamScheduleResponse> {
    ...
    if (!response.ok) {
      const error = await response.json();
      throw new Error(error.detail || `Failed to update exam schedule: ${response.statusText}`);
    }
    return response.json();
  }
  ...
  async uploadExamScheduleImage(id: number, file: File): Promise<ExamScheduleResponse> {
  ...
  async getAdminStats(): Promise<{
    ...
    try {
      const [courses, programs, classSchedules, examSchedules] = await Promise.all([
