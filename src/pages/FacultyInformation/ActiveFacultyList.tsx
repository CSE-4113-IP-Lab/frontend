...
const ActiveFacultyList: React.FC = () => {
  ...
  useEffect(() => {
    const fetchActiveFaculties = async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error(`Failed to fetch active faculties: ${response.status} ${response.statusText}`);
        }

        const data = await response.json();
        
        // Filter only active faculties (not on leave)
        const activeFaculties = data.filter((faculty: Faculty) => faculty.on_leave === 0);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ backgroundColor: '#E8E9EA' }}>
        <div className="text-center">
          ...
          <p className="text-xl text-gray-600">Loading active faculties...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen" style={{ backgroundColor: '#E8E9EA' }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          ...
          <div className="text-center">
            <h1 className="text-4xl font-bold text-gray-900 mb-2">Active Faculty Members</h1>
            <p className="text-xl text-gray-600">
              {faculties.length} active faculty member{faculties.length !== 1 ? 's' : ''}
            </p>
          </div>
        </div>
        ...
        {faculties.length === 0 && (
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2">No active faculty members found</h3>
            <p className="text-gray-600">No active faculty members are currently available.</p>
          </div>
        )}
      </div>
    </div>
