...
import { Plus, MapPin, Calendar, Clock, Eye } from "lucide-react";
...
export default function RoomManagement() {
  ...
  const handleRollDailySlots = async () => {
    if (
      !confirm(
        "Are you sure you want to roll daily slots? This will move room schedules forward by one day."
      )
    ) {
      return;
    }
          ...
          <div className="flex justify-between items-center py-6">
            <div>
              ...
              <p className="text-gray-600">
                Manage rooms, schedules, and bookings
              </p>
            </div>
            <Button
                      ...
                      {rooms.map((room) => (
                        <div key={room.id} className="border rounded-lg p-4">
                          ...
                          <p className="text-sm text-gray-600 mb-3">
                            <strong>Hours:</strong>{" "}
                            {RoomService.formatTime(room.operating_start_time)}{" "}
                            - {RoomService.formatTime(room.operating_end_time)}
                          </p>
                          <div className="flex gap-2">
                            <Button
                              variant="outline"
                              ...
                              onClick={() =>
                                navigate(
                                  `/room-booking/book?room_id=${room.id}`
                                )
                              }
                            >
                            ...
                            <Button
                              ...
                              size="sm"
                              onClick={() =>
                                navigate(`/room-booking/room/${room.id}`)
                              }
                            >
                      ...
                      <div key={booking.id} className="border rounded-lg p-4">
                        <div className="flex justify-between items-start">
                          <div className="flex-1">
                            ...
                            <p className="text-sm text-gray-600 mb-1">
                              <strong>Date:</strong>{" "}
                              {RoomService.formatDate(booking.booking_date)}
                            </p>
                            <p className="text-sm text-gray-600 mb-1">
                              <strong>Time:</strong>{" "}
                            ...
                            <p className="text-sm text-gray-600 mb-1">
                              ...
                              {booking.duration_slots * 30} minutes
                            </p>
                            {booking.user_name && (
                              <p className="text-sm text-gray-600 mb-1">
                              ...
                              <Button
                                ...
                                onClick={async () => {
                                  if (
                                    ...
                                    try {
                                      await RoomService.cancelBooking(
                                        booking.id
                                      );
