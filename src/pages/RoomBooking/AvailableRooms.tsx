...
const AvailableRooms: React.FC<AvailableRoomsProps> = () => {
  ...
  const searchAvailableRooms = async () => {
    ...
    try {
      ...
      const errorMessage =
        err.response?.data?.detail || "Failed to search rooms";
  ...
  const validateSearchForm = (): string | null => {
    ...
    if (bookingDate >= oneWeekFromNow)
      return "Booking date must be within the next 7 days";

    const [startHour, startMinute] = searchParams.start_time
      .split(":")
      .map(Number);
  ...
  const handleInputChange = (
    field: keyof AvailableRoomsRequest,
                      ...
                      searchParams.end_time && (
                        <div className="bg-blue-50 p-3 rounded-md mb-4">
                          <p className="text-sm text-blue-700">
                            ...
                            {RoomService.formatDate(
                              searchParams.booking_date
                            )}{" "}
                            {RoomService.formatTime(searchParams.start_time)} -{" "}
                            {RoomService.formatTime(searchParams.end_time)}
                          </p>
                        </div>
                      )}
